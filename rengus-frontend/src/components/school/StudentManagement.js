...
const StudentManagement = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    birth_country: '',
    address: '',
    school_id: currentUser?.school_id || '', // Auto-populate with current user's school
    status: 'active',
    nip: '',
    cnss_number: '',
    cnamgs_number: '',
  ...
  const fetchStudents = async () => {
    try {
      // For school users, only fetch students from their school
      const response = await studentsAPI.getBySchool(currentUser.school_id);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      // For school users, ensure the school_id is always set to their school
      ...
      submissionData.qr_code = qrCodeData;
